...
class DUALGAN():
    ...
    def build_generator(self):
        ...
        model = Sequential()
        model.add(Dense(128, input_dim=self.img_dim,activation=tf.nn.leaky_relu))
        ...
        model.add(BatchNormalization(momentum=0.8))
        model.add(Dropout(0.4))
        model.add(Dense(self.img_dim, activation=tf.nn.leaky_relu))
    ...
    def build_discriminator(self):
        ...
        model = Sequential()
        model.add(Dense(1024, input_dim=self.img_dim,activation=tf.nn.leaky_relu))
        model.add(BatchNormalization(momentum=0.8))
        model.add(Dense(512,activation=tf.nn.leaky_relu))
        model.add(BatchNormalization(momentum=0.8))
        model.add(Dense(256,activation=tf.nn.leaky_relu))
    ...
    def train(self, epochs, batch_size=128, sample_interval=50):
        ...
        # Rescale -1 to 1
        X_train = (2*(X_train.astype(np.float32) - np.max(X_train)) / np.max(X_train)) -1

        # Domain A and B (rotated)
        X_A = X_train[:int(X_train.shape[0]/2)]
#        X_B = scipy.ndimage.interpolation.rotate(X_train[int(X_train.shape[0]/2):], 0, axes=(1, 2))
        X_B = X_train[:int(X_train.shape[0]/2)]
...
if __name__ == '__main__':
    gan = DUALGAN()
    gan.train(epochs=30000000, batch_size=128, sample_interval=200)
