...
class AdversarialAutoencoder():
    ...
    def build_encoder(self):
        ...
        h = Flatten()(img)
        h = Dense(1024,activation=tf.nn.leaky_relu)(h)
        ...
        h = Dense(64,activation=tf.nn.leaky_relu)(h)
        h = Dense(32,activation=tf.nn.leaky_relu)(h)
        h = Dense(16,activation=tf.nn.leaky_relu)(h)
    ...
    def build_discriminator(self):
        ...
        model.add(Dense(1024, input_dim=self.latent_dim,activation=tf.nn.leaky_relu))
        model.add(Dense(512,activation=tf.nn.leaky_relu))
        model.add(Dense(256,activation=tf.nn.leaky_relu))
...
if __name__ == '__main__':
    ...
    aae.train(epochs=1000000, batch_size=32, sample_interval=200)
