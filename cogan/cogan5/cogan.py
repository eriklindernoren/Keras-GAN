...
class COGAN():
    ...
    def build_generators(self):
        ...
        model = Sequential()
        model.add(Dense(4000, input_dim=self.latent_dim,activation=tf.nn.leaky_relu))
#        model.add(LeakyReLU(alpha=0.2))
        model.add(BatchNormalization(momentum=0.8))
        model.add(Dense(3000,activation=tf.nn.leaky_relu))
        model.add(BatchNormalization(momentum=0.8))
        model.add(Dense(2000,activation=tf.nn.leaky_relu))
        model.add(BatchNormalization(momentum=0.8))
        model.add(Dense(1500,activation=tf.nn.leaky_relu))
        model.add(BatchNormalization(momentum=0.8))
