...
## About
Keras implementations of Generative Adversarial Networks (GANs) suggested in research papers. Since I would like to enable people without GPUs to test these implementations out I will often prefer dense layer over convolutional layers if they get the job done. These models are in some cases simplified versions of the ones ultimately described in the papers, but I have chosen to focus on getting the core ideas covered instead of getting every layer configuration right. However, because of this the results will not always be as nice as in the papers.
