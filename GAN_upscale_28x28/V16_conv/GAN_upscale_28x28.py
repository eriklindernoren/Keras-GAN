...
#        model.add(Input(shape=(16,)))
        model.add(Reshape(input_shape=(16,),target_shape=(4,4,1)))
        model.add(Conv2DTranspose(64,(2,2),padding='same'))
        model.add(LeakyReLU(alpha=0.2))
        model.add(BatchNormalization(momentum=0.8))
        model.add(Conv2DTranspose(128,(2,2),padding='same'))
        model.add(LeakyReLU(alpha=0.2))
        model.add(BatchNormalization(momentum=0.8))
        model.add(Dense(512))
        model.add(LeakyReLU(alpha=0.2))
...
#        model.add(keras.layers.GaussianNoise(0.0001,))
        model.add(Conv2D(16,(3,3),input_shape=self.img_shape))
        model.add(LeakyReLU(alpha=0.2))
        model.add(Conv2D(32,(3,3),input_shape=self.img_shape))
        model.add(LeakyReLU(alpha=0.2))
        model.add(Flatten())
        model.add(Dense(512))
        model.add(LeakyReLU(alpha=0.2))
...
if __name__ == '__main__':
    gan = GAN()
    gan.train(epochs=30000000000, batch_size=100, sample_interval=100)